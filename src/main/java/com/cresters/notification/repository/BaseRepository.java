...
import com.cresters.notification.domain.enums.Status;
import com.cresters.notification.domain.request.PaginationRequest;
...
public class BaseRepository extends CustomRepository {
    ...
    public <T> Map<String, Object> getRouteInfo(T entity) {
        Map<String, Object> approvalRoute = new HashMap<>();
        em.detach(entity);
        try {

            Field recId = entity.getClass().getDeclaredField("id");
            recId.setAccessible(true);

            if (!Objects.equals(recId.get(entity), null)) {
                Long recordId = Long.parseLong(String.valueOf(recId.get(entity)));
                approvalRoute.put("recordId", recordId);
            }

        } catch (NoSuchFieldException | IllegalAccessException e) {
            throw new BadRequestException("Authorization configuration error");
        }
